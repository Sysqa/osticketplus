...
if (defined('THIS_VERSION') && strcasecmp($cfg->getVersion(), THIS_VERSION)) {
    $sysnotice = sprintf(_('The script is version %s while the database is version %s.'), THIS_VERSION, $cfg->getVersion());
    if (file_exists('../setup/'))
        $sysnotice.=_('Possibly caused by incomplete <a href="../setup/upgrade.php">upgrade</a>.');
        ...
        case 'attach':
            ...
            if (!$errors) {
                ...
                if (db_query($sql)) {
                    ...
                    $errors['err'] = _('Update error!');
                }
            } else {
                $errors['err'] = _('Error occured. See error messages below.');
            }
            break;
        case 'api':
...
switch ($thistab) {
    ...
    case 'staff':
        ...
        $nav->addSubMenu(array('desc' => _('Add New User'), 'href' => 'admin.php?t=staff&a=new', 'iconclass' => 'newuser'));
        $nav->addSubMenu(array('desc' => _('User Groups'), 'href' => 'admin.php?t=groups', 'iconclass' => 'groups'));
        $nav->addSubMenu(array('desc' => _('Add New Group'), 'href' => 'admin.php?t=groups&a=new', 'iconclass' => 'newgroup'));
        $page = '';
        switch ($thistab) {
            case 'grp':
            case 'groups':
            ...
            case 'staff':
                ...
                if (($id = $_REQUEST['id'] ? $_REQUEST['id'] : $_POST['staff_id']) && is_numeric($id)) {
                    ...
                    if (!$staff || !is_object($staff) || $staff->getId() != $id) {
                        $staff = null;
                        $errors['err'] = sprintf(_('Unable to fetch info on rep ID#%s'), $id);
                    }
                }
...
?>
                <p align="center">
                    <font class="error"><?= _('Problems loading requested admin page.') ?> (<?= Format::htmlchars($thistab) ?>)</font>
