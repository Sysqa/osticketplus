...
if ($_POST && !$errors):
    ...
    if ($ticket && $ticket->getId()) {
        ...
        switch (strtolower($_POST['a'])):
            case 'reply':
                $fields = array();
                $fields['msg_id'] = array('type' => 'int', 'required' => 1, 'error' => _('Missing message ID'));
                ...
                if ($_FILES['attachment'] && $_FILES['attachment']['size']) {
                    if (!$_FILES['attachment']['name'] || !$_FILES['attachment']['tmp_name'])
                        $errors['attachment'] = _('Invalid attachment');
                    ...
                    elseif (!$cfg->canUploadFileType($_FILES['attachment']['name']))
                        $errors['attachment'] = _('Invalid file type');
                }
...
if ($cfg->showAnsweredTickets()) {
    $nav->addSubMenu(array('desc' => sprintf(_('Open (%s)'), ($stats['open'] + $stats['answered']))
            , 'title' => _('Open Tickets'), 'href' => 'tickets.php', 'iconclass' => 'Ticket'));
} else {
    if ($stats['open'])
        $nav->addSubMenu(array('desc' => sprintf(_('Open Tickets (%s)'), $stats['open']), 'title' => _('Open Tickets'), 'href' => 'tickets.php', 'iconclass' => 'Ticket'));
    if ($stats['answered']) {
        $nav->addSubMenu(array('desc' => sprintf(_('Answered (%s)'), $stats['answered']),
            'title' => _('Answered Tickets'), 'href' => 'tickets.php?status=answered', 'iconclass' => 'answeredTickets'));
    }
}
