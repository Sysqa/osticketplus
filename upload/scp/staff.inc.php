...
if (basename($_SERVER['SCRIPT_NAME']) == basename(__FILE__))
    die('Kwaheri rafiki!'); //Say hi to our friend..
...
if (defined('THIS_VERSION') && strcasecmp($cfg->getVersion(), THIS_VERSION)) {
    $errors['err'] = $sysnotice = sprintf(_('The script is version %s while the database is version %s'), THIS_VERSION, $cfg->getVersion());
} elseif ($cfg->isHelpDeskOffline()) {
    $sysnotice = _('<strong>System is set to offline mode</strong> - Client interface is disabled and ONLY admins can access staff control panel.');
