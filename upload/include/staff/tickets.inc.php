<?php
if(!defined('OSTSCPINC') || !@$thisuser->isStaff()) die(_('Access Denied'));
...
<div id='basic' style="display:<?=$basic_display?'block':'none'?>">
    ...
    <table>
        <tr>
            ...
            <td><input type="submit" name="basic_search" class="button" value="<?= _('Search') ?>">
                &nbsp;[<a href="#" onClick="showHide('basic','advance'); return false;"><?= _('Advanced') ?></a> ] </td>
        </tr>
    </table>
    </form>
</div>
<div id='advance' style="display:<?=$basic_display?'none':'block'?>">
 ...
 <input type="hidden" name="a" value="search">
  <table>
    <tr>
        <td><?= _('Query:') ?> </td><td><input type="text" id="query" name="query" value="<?=Format::htmlchars($_REQUEST['query'])?>"></td>
        <td><?= _('Dept:') ?></td>
        ...
        if($tickets_res && ($num=db_num_rows($tickets_res))):
            ...
            <tr class="<?=$class?> " id="<?=$row['ticket_id']?>">
                ...
                <td align="center" title="<?=$row['email']?>" nowrap>
                  <a class="Icon <?=strtolower($row['source'])?>Ticket" title="<?=$row['source']?> <?= _('Ticket:') ?> <?=$row['email']?>"
