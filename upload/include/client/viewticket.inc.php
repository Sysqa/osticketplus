<?php
if(!defined('OSTCLIENTINC') || !is_object($thisclient) || !is_object($ticket)) die(_('Kwaheri rafiki!')); //bye..see ya
...
<table width="100%" cellpadding="1" cellspacing="0" border="0">
    <tr><td colspan=2 width=100% class="msg"><?=_('Ticket #') ?><?=$ticket->getExtId()?> 
        &nbsp;<a href="view.php?id=<?=$ticket->getExtId()?>" title="<?=_('Reload') ?>"><span class="Icon refresh">&nbsp;</span></a></td></tr>
    <tr>
       <td width=50%>	
...
<div class="msg"><?=_('Subject:') ?> <?=Format::htmlchars($ticket->getSubject())?></div>
<div>
    <?if($errors['err']) {?>
...
<div>
    ...
    <div id="reply" style="padding:10px 0 20px 40px;">
        ...
        <form action="view.php?id=<?=$id?>#reply" name="reply" method="post" enctype="multipart/form-data">
            ...
            <div align="left">
                <?=_('Enter Message') ?> <font class="error">*&nbsp;<?=$errors['message']?></font><br/>
                <textarea name="message" id="message" cols="60" rows="7" wrap="soft"><?=$info['message']?></textarea>
            </div>
            <? if($cfg->allowOnlineAttachments()) {?>
            <div align="left">
                <?=_('Attach File') ?><br><input type="file" name="attachment" id="attachment" size=30px value="<?=$info['attachment']?>" />
            ...
            <div align="left"  style="padding:10px 0 10px 0;">
                ...
                <input class="button" type='button' value='<?=_('Cancel') ?>' onClick='window.location.href="view.php"' />
            </div>
        </form>
    </div>
</div>
